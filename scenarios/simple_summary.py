...
A minimal scenario — just prints basic stats. Used to verify the
multi-scenario selection mechanism works correctly.
...
def run(data: dict, params: dict = None):
    ...
    for key, df in data.items():
        ...
        if "RSI" in df.columns:
            results[key]["avg_RSI"] = round(df["RSI"].mean(), 2)
        if "ATR" in df.columns:
            results[key]["avg_ATR"] = round(df["ATR"].mean(), 2)
    return results
